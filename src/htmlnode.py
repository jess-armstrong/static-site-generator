...
class LeafNode(HTMLNode):
    ...
    def to_html(self):
        if self.value == None:
            raise ValueError("No value. LeafNode must have a value")
        if self.tag == None:
            return self.value
...
class ParentNode(HTMLNode):
    ...
    def to_html(self):
        if self.tag == None:
            raise ValueError("No tag. ParentNode must have a tag")
        if self.children == None:
            raise ValueError("No children. ParentNode must have children")
        children_str = ""
